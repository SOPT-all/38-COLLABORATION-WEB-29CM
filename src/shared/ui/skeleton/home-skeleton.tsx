const HomeMenuSkeleton = () => (
  <div className="flex h-[42px] items-end gap-[15px] px-9 pb-[10px]">
    {Array.from({ length: 7 }).map((_, i) => (
      <div key={i} className="h-4 w-12 animate-pulse bg-gray-200" />
    ))}
  </div>
);
...
const ShortcutSectionSkeleton = () => (
  <section className="grid grid-cols-6 gap-x-4.5 gap-y-5 px-9.5 py-18">
    {Array.from({ length: 12 }).map((_, i) => (
      <div key={i} className="aspect-2/1 w-full animate-pulse bg-gray-200" />
    ))}
  </section>
);
...
const ProductSectionSkeleton = () => (
  <section className="flex gap-6">
    <div className="aspect-578/680 h-[calc(100vh-84px)] shrink-0 animate-pulse bg-gray-200" />
